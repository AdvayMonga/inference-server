...
`check_e4b_stop_tokens_modal.py` is a one-shot Modal instrument, not part of the gate: it closed
the E4B half of `kb-20260918-5906bc13` by replaying the June head-to-head's 8 raw token-id
prompts on `google/gemma-4-E4B-it` alone (plain `transformers`, greedy, 100 steps, nothing
stopping early) and printing where `1` / `106` / `50` first appear, so the two engines' stop sets
can be compared on the same rollout. Cheapest card that fits (L4, 24 GB), weights on the
`hf-cache` volume; `--dry-run` prints the plan without starting a GPU. Needs the `modal` extra.
