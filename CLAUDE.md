...
| `control_plane/prefix_index.py` | Global prefix index — block-aligned prompt prefix → replicas believed to hold it, `insert` / `lookup` / `evict` / `drop_replica`. Mirrors `PrefixCache.lookup` without importing it — both its semantics and its `_lengths` skip, which is what keeps a 32k-token lookup at ~0.2 ms instead of the 157 ms a key per block boundary costs. Entries carry the tick they were last confirmed at; the index never updates itself, so staleness is a dial an experiment sets |
...
| `launch_table.py` | Persisted Triton launch configs (`models/launch_table.py`), loaded at model load from `CUSTOM_BACKEND_LAUNCH_TABLE`; no entry = the default launch. Filled by `scripts/bench/tune_triton_launch.py` |
...
- **Substrate.** Modal is out — credits exhausted 2026-09-07, and gVisor plus alpha multi-GPU-incompatible snapshots make it the wrong platform for a cold-start thesis (`kb-20260916-d12c9170`). Rented pods via `research/venues.py` are the executor today; whether SLURM is worth a second launcher, and whether it has node-local NVMe and a fast interconnect (decides migrate-vs-recompute in Phase 6).
...
**Next:** Phase 4, gated on the Phase 0 decisions. The first GPU job is the simulator's hardware check, built and dry-run-tested but not yet rented (needs `RUNPOD_API_KEY`; smoke the venue first with `scripts/tools/venue_smoke.py`): `scripts/tools/run_on_runpod.py scripts/bench/replay_corpus_runpod.py` serves the engine on the pod with `TELEMETRY_DIR` on and replays `steady_interactive/seen` at rate x1, x2, x4 → `python scripts/tools/fit_timing_from_runs.py <run_group>` fits the `TimingModel` into `knowledge/timing/` → `... --validate` simulates the same configs and prints the `rank_correlation` vs hardware, to be filed in `knowledge/` by hand. Recipe in `scripts/README.md`.
**GPU budget:** rented pods, one venue. The `*_modal.py` instruments were archived on 2026-09-16 under `scripts/archive/modal/` (kept because 13 knowledge entries cite them as provenance; not expected to run) along with `modal_app.py`; `run_instrument.sh` stays at `scripts/` root as their launcher. New GPU work runs cheap tiers locally first and only then on a rented pod through `research/venues.py`. The CI `gpu` lane runs `scripts/gpu_tests/cuda_gate.py` the same way (`RUNPOD_API_KEY` repo secret, no fallback) with a pod reaper on `if: always()`.
