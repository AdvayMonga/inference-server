...
- **Knowledge base:** `knowledge/*.json` is the source of truth. `DECISIONS.md` is a GENERATED
  view — edit the JSON, then `loop index`. Experiments land in `experiments/*.json`; raw panels
  in `runs/` (gitignored).
...
- ✅ **Load-test client** — `scripts/load_test.py` + `scripts/plot_load_test.py`
