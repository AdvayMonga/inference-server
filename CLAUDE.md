...
| `SchedulingPolicy` | `scheduling_policy.py` — pure `fcfs_order` / `fair_order` / `fair_initial_counter` / `fair_charge` over any `(session_id, priority, arrival_seq)` record; `FCFSPolicy` / `FairPolicy` are thin stateful shells. Simulator and scheduler share them. |
