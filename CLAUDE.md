...
**GPU budget:** Modal credits ran out on 2026-09-07. The `*_modal.py` instruments and `run_instrument.sh` still exist; new GPU work runs cheap tiers locally first and only then on a rented pod through `research/venues.py`. The CI `gpu` lane runs `scripts/gpu_tests/cuda_gate.py` the same way (`RUNPOD_API_KEY` repo secret; Modal tokens as fallback) with a pod reaper on `if: always()`.
