...
| `chunked_prefill_ab.csv` | A10G / E2B | `scripts/archive/modal/bench/bench_chunked_prefill_modal.py` | chunked vs monolithic prefill: head-of-line stall 12x |
| `sweep_custom_cuda.csv`, `sweep_cuda.csv`, `sweep_vllm.csv` | A10G / E2B | `scripts/archive/modal/bench/bench_load_sweep_modal.py`, `scripts/archive/modal/bench/bench_vllm_sweep_modal.py` | first head-to-head: ours vs HF vs vLLM |
| `sweep_custom_cuda_batched.csv` | A10G / E2B | `scripts/archive/modal/bench/bench_load_sweep_modal.py` | batched prefill: TTFT 1841 → 213 ms |
...
| `serving_a100_e4b*.csv` | A100-80GB / E4B | `scripts/archive/modal/bench/bench_serving_modal.py` | open-loop Poisson serving sweeps (rate → throughput, TTFT, TPOT, SLO) |
...
prefix sharing, scheduler-driven) on an **A10G**, model **google/gemma-4-E2B-it**, bf16.
...
A/B of `PREFILL_MODE` (`scripts/archive/modal/bench/bench_chunked_prefill_modal.py`, A10G/E2B, in-process scheduler).
...
- **custom** — our engine (`scripts/archive/modal/bench/bench_load_sweep_modal.py`, `custom-cuda`).
- **HF** — `AutoModelForCausalLM` baseline (`cuda`) under our *same* scheduler.
- **vLLM** — vLLM 0.23 V1 in-process (`scripts/archive/modal/bench/bench_vllm_sweep_modal.py`), full opts (CUDA graphs +
...
The head-to-head TTFT gap was diagnosed (`scripts/archive/modal/probes/profile_prefill_modal.py`) as **eager, serial
