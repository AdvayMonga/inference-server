"""Build blind cards + answer key. Outcome scoring follows PREREGISTRATION.md mechanically."""
import json, random, pathlib
REPO = pathlib.Path(__file__).resolve().parents[2]
OUT = REPO / "research/bakeoff"

LOWER_BETTER = {"ttft_prefill_p95", "ttft_prefill_p50", "ttft_p95", "ttft_p50", "decode_ms_by_bucket",
                "graph_capture_s", "total_iteration_errors", "wall_s"}
HIGHER_BETTER = {"tok_s_within_slo", "cache_hit_rate", "cache_max_blocks"}

def load(eid):
    return json.loads((REPO / f"experiments/{eid}.json").read_text())
...
def score(rec):
    ...
    sig = rec["gates"]["significance"]
    reason = sig.get("reason", "").lower()
    if not sig.get("passed") or reason.startswith("instrument change") or reason.startswith("instrument_change") \
            or "no measurable change, as intended" in reason:
        return 1
    metric, d = next(iter(rec["delta"].items()))
    pct = d.get("pct")
    if pct is None:
        improving = (d["after"] > d["before"]) if metric in HIGHER_BETTER else (d["after"] < d["before"])
        return 3 if improving else 2
    improving = pct > 0 if metric in HIGHER_BETTER else pct < 0
    assert metric in HIGHER_BETTER | LOWER_BETTER, metric
    return 3 if improving and abs(pct) >= 30 else 2

A100 = "A100-80GB, Gemma 4 E4B"
CARDS = [
 dict(key="tiled", records=["exp-20260905-520667e1", "exp-20260905-e5da5d8a", "exp-20260905-26c0e0f5"],
  category="kernel",
  ...
  context=f"{A100}; serving harness with a cache-miss-heavy workload of variable-length prompts; A/B by toggling the kernel via configuration in one serving process; prefill CUDA graphs enabled.",
  tier="4 (full serving sweep)"),
 dict(key="blockm16", records=["exp-20260905-0d85ab30"], category="kernel",
  ...
  metric="TTFT prefill-phase p95 (lower is better)",
  context=f"{A100}; serving harness, cache-miss-heavy workload; one run per arm; A/B by configuration toggle.",
  tier="4 (full serving sweep)"),
 dict(key="graphvariants", records=["exp-20260905-fcc4f992", "exp-20260905-4a8ad526", "exp-20260905-927c75ff", "exp-20260905-bc4f63f8"],
  ...
  context=f"{A100}; serving harness, cache-miss-heavy workload; replicated runs per arm.",
  tier="4 (full serving sweep)"),
 dict(key="skipinit", records=["exp-20260917-2ee69bde"], category="model_load",
 ...
 dict(key="mixed", records=["exp-backfill-feat-mixed-batch-prefill"], category="scheduling",
  ...
  context=f"{A100}; decode steps are normally replayed from bucketed CUDA graphs; premise probe comparing step time of today's decode step against the mixed step.",
  tier="3 (single-GPU probe)"),
 dict(key="radix", records=["exp-backfill-feat-radix-prefix-cache"], category="caching",
  ...
  tier="2 (CPU repro)"),
 dict(key="kvbp", records=["exp-backfill-fix-kv-backpressure-and-cache-eviction"], category="robustness",
  ...
  context=f"{A100}; stress harness, distinct prompts, small KV pools, arrival rates past the saturation knee.",
  tier="4 (full stress sweep)"),
 dict(key="ima", records=["exp-backfill-fix-prefill-graph-ima"], category="cuda_graph",
  ...
  context=f"{A100}; prefill graph probe with repeated replays and block alloc/free churn between them.",
  tier="3 (single-GPU probe)"),
 dict(key="radcap", records=["exp-backfill-fix-radix-capacity-semantics"], category="caching",
  ...
  context=f"{A100}; high-concurrency serving run at the engine's stated concurrency target.",
  tier="2 (CPU repro)"),
 dict(key="obs", records=["exp-backfill-obs-ttft-breakdown"], category="cuda_graph",
  ...
  context=f"{A100}; serving sweep over Poisson arrival rates, prompt-length distribution resembling chat traffic.",
  tier="4 (full serving sweep)"),
 dict(key="bucketdec", records=["exp-backfill-perf-bucketed-decode-graphs"], category="cuda_graph",
  ...
  context=f"{A100}; in-process decode A/B against a forced max-size bucket, with a graphed-vs-eager parity check.",
  tier="3 (single-GPU probe)"),
 dict(key="depy", records=["exp-backfill-perf-depython-decode-step"], category="launch_overhead",
  ...
  context=f"{A100}; serving sweep at SLO-constrained load.",
  tier="4 (full serving sweep)"),
 dict(key="captureorder", records=["exp-backfill-perf-graph-capture-order"], category="compile",
  ...
  context=f"{A100}; MAX_BATCH_SIZE=256, an eight-bucket ladder, torch.compile on.",
  tier="3 (single-GPU probe)"),
 dict(key="dyncompile", records=["exp-backfill-perf-graph-compile-cost"], category="compile",
  ...
  metric="Total decode graph capture/compile time at startup, seconds (lower is better)",
  context=f"{A100}; a five-to-six-bucket ladder, torch.compile on.",
  tier="3 (single-GPU probe)"),
 dict(key="inductor", records=["exp-backfill-perf-inductor-cache-volume"], category="compile",
  ...
  context=f"{A100}; MAX_BATCH_SIZE=32, torch.compile on, serverless container with a mounted volume.",
  tier="3 (single-GPU probe)"),
 dict(key="waves", records=["exp-backfill-perf-length-grouped-waves"], category="scheduling",
  ...
  metric="Throughput within the latency SLO, tokens/s (higher is better)",
  context=f"{A100}; serving sweep with Poisson arrivals, MAX_BATCH_SIZE=256, torch.compile off, batched prefill mode; grouping on vs off.",
  tier="4 (full serving sweep)"),
 dict(key="lmhead", records=["exp-backfill-perf-lm-head-last-position"], category="compute_reduction",
  ...
  context=f"{A100}; isolated prefill forward with the slicing on vs off.",
  tier="3 (single-GPU probe)"),
 dict(key="prefillgen", records=["exp-backfill-perf-prefill-graph-general"], category="launch_overhead",
  ...
  context=f"{A100}; prefill overhead probe; the code path is CUDA-specific.",
  tier="3 (single-GPU probe)"),
 dict(key="longprompts", records=["exp-backfill-perf-prefill-graph-long-prompts"], category="cuda_graph",
  ...
  context=f"{A100}; serving sweep with a chat-like prompt-length distribution and a warm prefix cache.",
  tier="4 (full serving sweep)"),
 dict(key="prefixhits", records=["exp-backfill-perf-prefill-graph-prefix-hits"], category="cuda_graph",
  ...
  context=f"{A100}; serving sweep, torch.compile off, workload of reused prompts with a warm prefix cache; prefill graph on vs off.",
  tier="4 (full serving sweep)"),
 dict(key="splitk", records=["exp-backfill-perf-split-k-decode-attention"], category="kernel",
  ...
  context=f"{A100}; isolated kernel test at a 2048-token context, correctness checked against the existing kernel.",
  tier="3 (single-GPU probe)"),
]
...
def main():
    ...
    for n, i in enumerate(order, 1):
        c = CARDS[i]; cid = f"C{n:02d}"
        recs = sorted((load(e) for e in c["records"]), key=lambda r: r["started_at"])
        latest = recs[-1]
        cards.append({"id": cid, "change": c["change"], "gap": c["gap"], "mechanism": c["mechanism"],
                      "metric": c["metric"], "context": c["context"], "tier": c["tier"],
                      "category": c["category"]})
        metric, d = next(iter(latest["delta"].items()))
        key[cid] = {"experiment_ids": [r["id"] for r in recs], "governing_record": latest["id"],
                    "branch": latest["branch"], "verdict": latest["verdict"],
                    "delta": {metric: {k: d.get(k) for k in ("before", "after", "pct")}},
                    "outcome_score": score(latest)}
    (OUT / "cards.json").write_text(json.dumps(cards, indent=2) + "\n")
    (OUT / "answer_key.json").write_text(json.dumps(key, indent=2) + "\n")
    for cid, k in key.items(): print(cid, k["governing_record"], k["verdict"], k["outcome_score"])
